...
def unique_suffix_alpha(length: int = 4) -> str:
    """숫자 없이 영문 대문자만 랜덤 접미사 생성 (예: ABCD)"""
    return unique_suffix(length=length, alphabet=string.ascii_uppercase)
...
def make_alpha_name(prefix: str,
                    suffix_len: int = 4,
                    with_date: bool = False) -> str:
    """
    접두사 + (옵션)날짜 + 영문 대문자만 랜덤 접미사
    - with_date=False (예: TEST_PROJECT_ABCD)
    - with_date=True (예: TEST_PROJECT_20251107_ABCD)
    """
    suf = unique_suffix_alpha(suffix_len)
    if with_date:
        stamp = datetime.now().strftime('%Y%m%d')
        return f"{prefix}{stamp}_{suf}"
    return f"{prefix}{suf}"