...
def login_as_admin(page: Page, log: Optional[object] = None) -> Page:
    """
...
def login_as_iam(page: Page, log: Optional[object] = None, user_id: str = "") -> Page:
    """
    ...
    # auth.login_iam(user_id=user_id, password=iam_pw)
...
def open_project(page: Page, log: Optional[object] = None, project_name: str = "QA_TEST_PROJECT") -> Page:
    """
    프로젝트 목록에서 지정한 프로젝트로 진입하는 헬퍼.

    Args:
        page: Playwright 페이지 객체
        log: 외부에서 주입한 로거. 미지정 시 "open_project" 로거 사용
        project_name: 진입할 프로젝트 이름

    Returns:
        Page: 프로젝트 진입 후의 동일 Page 객체
    """
